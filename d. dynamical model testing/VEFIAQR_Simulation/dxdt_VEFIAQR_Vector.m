function dxdt = dxdt_VEFIAQR_Vector(x,t,params)
...
tempID = 1:32;
V = reshape(x(tempID + 32*0), [8,4]);
E = reshape(x(tempID + 32*1), [8,4]);
F = reshape(x(tempID + 32*2), [8,4]);
I = reshape(x(tempID + 32*3), [8,4]); 
A = reshape(x(tempID + 32*4), [8,4]);
Q = reshape(x(tempID + 32*5), [8,4]);
R = reshape(x(tempID + 32*6), [8,4]);
